...
def load_data():
    ...
    movielens_path = os.path.join(data_path, movielens_dir_nm)

    field_dims = np.load(os.path.join(data_path, 'field_dims_mlp.npy'))
    dropout= 0.4
    embed_dim= 16
    ...
    input_data = np.array([[0 for _ in range(len(field_dims))]])
    model(input_data)
    model.load_weights(os.path.join(model_path, 'autoIntMLP_model.weights.h5'))
    label_encoders = joblib.load(os.path.join(data_path, 'autoIntMLP_label_encoders.pkl'))
